...
import { fetchBooks, Book, getBookCoverUrl } from '../lib/books';
...
const BookCard = React.memo(({ book, onBookClick }: { book: Book, onBookClick: (id: string) => void }) => {
  ...
  }, [book.title, user, handleAuthRedirect]);
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          <button 
            className="p-3 bg-white rounded-full hover:bg-blue-500 hover:text-white will-change-transform transition-colors duration-200 transform hover:scale-110 hover:-translate-y-1"
          ...
          >
            <Heart className={`h-5 w-5 ${book.likes_count > 0 ? 'fill-red-500 text-red-500' : ''}`} />
          </button>
        </div>
        {book.likes_count > 0 && (
